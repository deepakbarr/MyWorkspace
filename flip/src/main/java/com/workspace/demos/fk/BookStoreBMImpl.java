...
import com.workspace.demos.fk.dao.impl.SearchBMImpl;
 ...
 */
public class BookStoreBMImpl implements BookStore {

    private Logger logger = Logger.getLogger(BookStoreBMImpl.class.getName());
    private static BookStoreBMImpl bookStore = null;

    private BookStoreBMImpl() {
    }

    public static BookStoreBMImpl getInstance() {
        if (null == bookStore) {
            bookStore = new BookStoreBMImpl();
        }
        return bookStore;
    }
    ...
    private List<Book> search(String[] keywords) {
        ...
        try {
            SearchDAO searchDAO = new SearchBMImpl();
